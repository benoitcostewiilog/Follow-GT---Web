
<div class="wrapper wrapper-content animated fadeInRight">
    <div id="tpltEdtHrchy" class="hidden">
        <ol class="breadcrumb">
            <li>
                <a href="#"><?php echo __('Historique'); ?></a>
            </li>
            <li>
                <a onclick="goBack();"><?php echo __('Urgences'); ?></a>
            </li>
            <li class="active">
                <strong><?php echo __('Créer un urgence'); ?></strong>
            </li>
        </ol>
    </div>
    <div class="row">
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <h5><?php echo __('Formulaire de création d\'un urgence'); ?></h5>
                </div>
                <div class="ibox-content">
                    <form method="POST" action="<?php echo url_for(array('sf_route' => 'urgence-create')) ?>" id="formNew" class="form-horizontal">
                        <?php include_partial('form', array( 'fournisseurs' => $fournisseurs, 'interlocuteurs' => $interlocuteurs, 'transporteurs' => $transporteurs,'chauffeurs'=>$chauffeurs, 'natures' => $natures)) ?>
                         <div class="form-group">
                            <div class="col-sm-4 col-sm-offset-2">
                                <button class="btn btn-white" onclick="goBack();" type="button"><?php echo __('Annuler'); ?></button>
                                <button class="btn btn-primary" type="submit"><?php echo __('Enregistrer'); ?></button>
                            </div>
                            <span class="form-mandatory-end"><span class="form-mandatory">* </span><?php echo __('champs obligatoires') ?></span>
                       </div>

                                 <span class="form-mandatory-end"><span class="form-mandatory">* </span>La déclaration d'une urgence doit etre effectuée au plus tard à J-1 avant la date d'arrivage</span>
                     
                    </form>
                </div>
            </div>
        </div>
    </div>
</div>
<script>
    $(document).ready(function () {
        //Maj des slots du layout
        $('#lytHrchy').html($('#tpltEdtHrchy').html());
        
        updateChauffeurSelect($('#transporteur').val());
    });
</script>